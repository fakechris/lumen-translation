-- Mirrors the official Bob PopClip extension: hand the text to LumenTranslation,
-- but also forward the user's PopClip option choices (engine, apiKey, model,
-- region, source/target lang) as a JSON record. LumenTranslation applies these
-- overrides before translating, so PopClip is the quick-switch UI and the
-- LumenTranslation Preferences window is the full management UI.
